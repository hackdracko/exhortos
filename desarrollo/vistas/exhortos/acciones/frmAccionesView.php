...
<div class="container panel panel-default">
    ...
    <input type="hidden" id="hddCveAccion">
    <div class="panel-body">
        <div id="divFormulario" class="form-horizontal">
            ...
            <div class="form-group">
                <label for="" class="caption control-label col-md-4 needed" id="anio">Descripci&oacute;n<span class="requerido">(*)</span></label>
                <div class="col-md-5">
                    <input type="text" class="form-control mayuscula" id="txtAccion" placeholder="Descripci&oacute;n">
                </div>
            </div>
...
<script type="text/javascript">
    validate = function () {
        var mensaje = "";
        var error = false;
        if ($('#txtAccion').val() == "" || $('#txtAccion').val() == "0") {
            $('#txtAccion').focus();
            $('#txtAccion').parent().append("<br class='required'><label class='Arial13Rojo required'>Ingrese una descripcion</label>");
            error = true;
        }
        return error;
    };
    ...
    save = function () {
        ...
        if (!validate()) {
            $.ajax({
                ...
                dataType: "json",
                data: {accion: "guardar",
                    cveAccion: $('#hddCveAccion').val(),
                    desAccion: $('#txtAccion').val(),
                    activo: 'S'
                },
                ...
                success: function (datos) {
                    if (datos.totalCount > 0) {
                        ...
                        $("#hddCveAccion").val(datos.data[0].cveAccion);
                        $("#txtAccion").val(datos.data[0].desAccion);
                        $("#btnEliminar").show();
                    } else {
        ...
        $.ajax({
            ...
            success: function (datos) {
                if (datos.totalCount > 0) {
                    ...
                    table += '<th>Clave de la acci&oacute;n</th>';
                    table += '<th>Descripci&oacute;n de la acci&oacute;n</th>';
                    table += '</tr>';
                    table += '</thead>';
                    table += "<tbody>";
                    for (var i = 0; i < datos.totalCount; i++) {
                        table += "<tr>";
                        table += "<td onclick='consutaId(" + datos.data[i].cveAccion + ")' >" + (i + 1) + "</td>";
                        table += "<td onclick='consutaId(" + datos.data[i].cveAccion + ")' >" + datos.data[i].cveAccion + "</td>";
                        table += "<td onclick='consutaId(" + datos.data[i].cveAccion + ")' >" + datos.data[i].desAccion + "</td>";
                        table += "</tr>";
                    }
                    table += "</tbody>";
                    table += "</table>";
